...
class DeploymentHelper:
    ...
    def _env_lines_systemd(self) -> str:
        ...
        if not self.env_vars:
            print("[deploy] ⚠ No env_vars passed. If your app needs DB creds, pass --env KEY=VALUE.")
            return ""
        lines = []
        for k, v in self.env_vars.items():
            # Wrap as "KEY=VALUE" to keep spaces safe.
    ...
    def _create_systemd_service(self, user: str = "root") -> None:
        ...
        # systemd unit files do NOT need shell quoting for ExecStart/WorkingDirectory
